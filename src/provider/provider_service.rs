...
pub fn get_weather(params: Vec<String>, provider: ProviderType) -> Result<Vec<Weather>, Box<dyn std::error::Error + Send + Sync>> {
    let provider = get_provider(provider);
    provider.get_weather(params)
}


pub fn describe_provider(provider: ProviderType) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    let provider = get_provider(provider);
    provider.describe()
}
